...
class MetricGanBrain(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        if self.sub_stage == SubStage.HISTORICAL:
            ...
            if self.hparams.degen_mode  == "CLEAN":
                noisy_wav, lens = batch.clean_sig
    ...
    def compute_objectives(self, predictions, batch, stage, optim_name=""):
        ...
        # D Learns to estimate the scores of enhanced speech
            ...
            #with open("enh_logger.txt","a") as f:
            #    for i in range(0,len(est_score)):
            #        #print("Name:%s Score:%s"%((ids[i]),(target_score[i])))
            #        f.write("Name:%s Score:%s\n"%((ids[i]),(target_score[i])))
            #    f.write("--------")
    ...
    def fit_batch(self, batch):
        ...
        if self.sub_stage == SubStage.CURRENT:
            for mode in ["clean", "enh", "noisy","denh"]:
            ...
            loss = self.compute_objectives(
                de_predictions, batch, sb.Stage.TRAIN, "D_denh"
            )
            self.d_optimizer.zero_grad()
            loss.backward()
            if self.check_gradients(loss):
    ...
    def on_stage_start(self, stage, epoch=None):
        ...
        if stage == sb.Stage.TRAIN:
            if self.hparams.target_metric == "pesq":
                self.target_metric = MetricStats(metric=pesq_eval, n_jobs=5,batch_eval=False)
            elif self.hparams.target_metric == "stoi":
        ...
        if stage != sb.Stage.TRAIN:
            self.pesq_metric = MetricStats(metric=pesq_eval, n_jobs=5,batch_eval=False)
    ...
    def init_optimizers(self):
        ...
        self.gd_optimizer = self.hparams.g_opt_class(
